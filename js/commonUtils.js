...
var mobile =!!(uagent.match(/(iPhone|iPod|blackberry|Nexus|android 0.5|htc|lg|midp|mmp|mobile|nokia|opera mini|palm|pocket|psp|sgh|smartphone|symbian|treo mini|Playstation Portable|SonyEricsson|Samsung|MobileExplorer|PalmSource|Benq|Windows Phone|Windows Mobile|IEMobile|Windows CE|Nintendo Wii)/i));
...
checkExternalUrl = function(url) {
    ...
    var previewLinkPattern = new RegExp('^((http(s)?://)?(labs|pre)?site(preview|builder)-[0-9]+(.(local|dev)?zohositescontent.com)/[a-zA-Z0-9\\x00-\\x7F\-\/]+((.html)|(/))(\\?(mobile|responsive)=true)?)$');
